...
- 結線：profiles に "harness.ops" を宣言した config（tmp_path 上に組み立てる）経由で load_profiles が ops を返し、
  invariant_checks に ci_lint.run_checks が入る＝verify の検査列に乗る。期待値は宣言した config と profile.py の構成
...
def test_load_profiles_includes_ops(tmp_path: Path) -> None:
    ...
    assert ci_lint.run_checks in loaded["ops"].invariant_checks
...
def test_ops_profile_light_import() -> None:
    ...
    code = (
        ...
        "assert harness.ops.PROFILE.invariant_checks == (ci_lint.run_checks,), harness.ops.PROFILE\n"
