...
def test_load_profiles_returns_ds_profile_for_this_repo() -> None:
    # このリポジトリの config は profiles = ["harness.ds", "harness.serve", "harness.agent", "harness.ops"]。
    # 4 つとも載る。
    root = Path(__file__).resolve().parents[1]
    loaded = profiles.load_profiles(root)
    assert [p.name for p in loaded] == ["ds", "serve", "agent", "ops"]
    ...
    assert ci_lint.run_checks in by_name["ops"].pm_checks  # CI テンプレートの構造 lint が繋がる
