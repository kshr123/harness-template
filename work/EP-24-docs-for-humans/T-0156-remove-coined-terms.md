...
## 追補（オーナー指摘で基準を修正）
「辞書語だから残す」「指紋→fingerprint と言い換える」だけでは不十分。**正しい基準は「読んだ人がその場で意味まで
分かるか」**で、単語の言い換えは解決でない。そこで専門用語（造語・英語・難しい和語のどれでも）には、初出に
**「何を意味し・なぜあるか」の平易な一文**を添えた（説明できないものは消す）。独立監査で MISSING/WEAK だった
ドメイン語に meaning-gloss を追加：fingerprint（中身から計算する短い識別子。中身が変われば必ず変わる…）・
manifest（保存物に付く由来書き）・champion（採用中の版）・registry/レジストリ（名前で部品を引ける登録簿）・
effort（推論の深さの指定）・交差検証・fold・ドリフト・由来/provenance・golden set・canary/shadow・WBS・
ミューテーション実測 ほか。汎用エンジニア語（docstring/JSONL/pickle/temperature 等）は対象外（対象読者に既知）。
