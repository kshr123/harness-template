...
def status_cmd(
    ...
    --next：着手候補（仕掛かり中の再開候補・着手できる末端単位・依存待ち・分解すべき outline epic）を端末に出す
    （各節は priority→ID 順・提案・門番でない）。
    """
    root = _root()
    if next_:
...
def check_main() -> None:
    ...
    def _run(
        ...
        scope: Annotated[
            str, typer.Option(help="all（全部）| diff（git 差分に応じて絞る参考実行。done の判定は verify）")
        ] = "all",
    ) -> None:
