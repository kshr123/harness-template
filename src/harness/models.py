...
class Status(StrEnum):
    """作業単位の状態。**それぞれ「誰の手番か」で分かれる**（進み具合の目盛ではない）。

    - `todo` … まだ始めていない。依存が全部 done なら「いま着手できる」候補として `--next` に出る。
    - `in_progress` … **いま手が動いている**。`--next` の先頭に出る（仕掛かり中＝再開の候補）。
    - `in_review` … **作る側の手は離れ、確かめる側の判断待ち**。自分では先へ進められない。
      「作る側と確かめる側を分ける」という原則を状態にしたもの。
    - `blocked` … 進めない（障害・外部待ち）。親の状態を畳むとき**最も強い**（1 つでもあれば親は blocked）。
    - `done` … 検証に成功した。**自己申告では置けない**（`verified_by` に実在するテストが要る）。

    見分け方：`in_progress` と `in_review` は**手が動いているか／人の判断を待っているか**。`in_review` と
    `blocked` は**待っているものが「確認」か、それ以外の障害か**。`todo` と `blocked` は**着手していない
    だけか／着手できないか**。

    **`in_review` と `blocked` だけが「人を呼ぶ状態」**で、`uv run status` の「人の判断待ち」に集約される
    （人はそこだけ見れば要所が来たと分かる）。
    """
...
class Item(BaseModel):
    ...
    # 同じ置き場の中での並び順（小さいほど先）。**人が決める情報**で、他の何からも導けない
    # （日程が無い単位もあるし、ID は採った順でしかない）ので、ここに置く。書かなければ従来どおり
    # ファイル名の順に並ぶ＝順序を気にしない案件は何も書かなくてよい。
    order: int | None = None
