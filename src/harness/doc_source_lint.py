...
設計の正本として参照すると、複製先でリンク切れになり、正本が一時物に乗る。根拠は本文の説明に置くべき。
...
- `artifacts/`・`results/`・`data/` は**契約のパス型**（`artifacts/serve/predictions/<名>/<日付>.jsonl` 等）で
  恒久ドキュメントに載るのが正しいので禁じない。作業単位 ID の provenance（learnings が「T-0095 で実装」
            ...
            for ref in _WORK_REF_RE.findall(line):
                problems.append(
                    pm.Problem(
                        ...
                        f"`work/` は複製で消える／置き換わるので、設計の根拠は本文の説明に置くこと"
                        f"（例：`work/EP-…/item.md の「やらないこと」` → 本文で理由を述べる）。"
