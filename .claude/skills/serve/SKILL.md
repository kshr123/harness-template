---
name: serve
description: 学習済みモデル（champion）を配信・サービングする際に自動参照。FastAPI で /predict・/health・/metadata を出し、予測を由来つき JSONL に記録する。配信・サービング・serve・推論 API・エンドポイント・デプロイ・FastAPI・Docker・K8s の語で発火。
---
...
4. 使う：GET /health（生存＋版）・GET /metadata（由来）・POST /predict（`{"records":[{列名:値}...]}`・
...
6. 配信後の分布ずれを見張る：`uv run data monitor --baseline <学習基準の表 id> [--auc] [--since YYYY-MM-DD]
   [--role primary|shadow|all] [--file-issue]`（予測 JSONL×学習基準の PSI/band。止める検査にせず band で読む＝exit 0。
