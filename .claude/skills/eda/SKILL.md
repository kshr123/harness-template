...
3. train/test があれば `uv run data compare <train_id> <test_id> [--auc]`（分布差・カテゴリ差・PSI／`--auc` で
   分布差 AUC）。※ compare は T-0026 で追加。
4. `--auc` が目安 0.7 以上なら分布シフトの疑い。原因列を `analysis.cv_permutation_importance` で特定し、
   除外・変換の仮説を experiment スキルへ渡す。※ T-0026/T-0027 で追加。
