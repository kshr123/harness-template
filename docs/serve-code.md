...
- **読者**：人（serve を保守・拡張する開発者／レビューするエージェント）
- **種別**：explanation（設計）
- **分かること**：`src/harness/serve/` の各ファイルの役割と組まれ方
- **ここからやること**：コードを読む前に全体像をつかむ
...
4. **実行基盤は利用者環境の関心**。トラフィック分割・スケール・耐障害には踏み込まず、壊れない**テンプレート**
   （`templates/serve/`）の提供までにする。その陳腐化は `deploy_lint` が構造検査で止める。
...
予測・予測ログ）に集約する。`deploy_lint.py` は実行経路の外＝配布テンプレートの陳腐化防止。
...
- `runtime.py` … 配信の実行時部品：champion の解決・予測・由来つき JSONL 予測ログ（`PREDICTION_LOG_FIELDS`・
...
- `deploy_lint.py` … 配信テンプレート（`templates/serve/`）の構造 lint。参照整合を verify で守る（実行しない資産の陳腐化防止）。
...
| shadow（新版の並走評価）の有効化 | 環境変数 `SERVE_SHADOW_*` | `app.py`（分岐） |
