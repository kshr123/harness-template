...
## 答え合わせの閉ループ（実測劣化→課題起票）

分布監視（`data monitor`）は**入力のずれの代理**しか見ない：入力の分布が安定したまま、現実の正解率だけが
崩れた champion は psi/drift では見えない。この穴を塞ぐのが `uv run data score`＝**後から届く実績（正解）と
配信時の予測を突き合わせて、実際に測った指標を出す**。

```
uv run data score --model <work/name> --actuals <実績テーブル> --actual-column <正解の列> [--file-issue]
```

- 突き合わせの鍵は `input_fingerprint`（予測 1 行に必ず付く決定的な指紋。features の正準 JSON の sha256）。
  実績テーブルは「この指紋 → 実際の正解」を持って後から用意する（利用者が業務 id から結合する）。
- 実測指標は `ds/eval` の evaluate 系（sklearn 素通し・`METRICS` レジストリ）で計算し、champion 昇格時の
  manifest metrics（＝約束した値）と比べて **相対劣化の band**（安定/要注意/大変化＝`DEGRADE_WATCH`/`ALERT`）を出す。
- **門番にしない**：drift 監視と同型で exit code は常に 0。`--file-issue` を付けると band=大変化の指標があるとき
  issues 登録簿へ**冪等に**起票する（答え合わせ指紋＝モデル×大変化の指標集合。劣化率・日付は含めない）。
  対処すると決めたら `promoted_to` で作業単位（再学習・特徴の見直し・切り戻し）へ結ぶ。
- champion が無い・実績テーブルが読めないときだけ非 0（黙って空で測らない）。serve は起動しない（結合は契約のみ）。

**保証しないこと（scope）**：正解ラベルが決して届かない配信（実績が事後にも得られない案件）では答え合わせは
成立しない＝この閉ループは動かせない。その場合に頼れるのは分布監視（代理）までである。劣化の band 閾値は
表示のための既定であって、「どれだけ悪化したら対処するか」は案件ごとの人の判断（機械の門番にしない）。
