...
- **読者**：人（新しい案件を立ち上げる／改良を本体へ戻す開発者）
- **種別**：how-to（複製と資産の流れの手順）
- **分かること**：この基盤を次の案件へ複製する手順と、案件↔本体の両方向の資産の流れ
- **ここからやること**：fork して案件領域を白紙化する（`uv run init-project`）

この文書は、この開発基盤を次の案件で使うときの**複製の仕方**と、複製後の**両方向の資産の流れ**の正本
（唯一の正とする置き場）。案件で得たものを次の案件へ渡し、案件で得た改良を本体へ戻す条件（AGENTS.md 目的 4。
この積み重ねの結果として「案件を重ねるほど強くなる」）を、機構を増やさず git の標準操作で成り立たせる。
