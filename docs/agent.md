...
- **検証（`uv run verify`）はネットワークを使わない**：実 API の代わりに dummy（合成応答）と cassette（実 API
  応答を JSON に固定してネットワークなしで再生する記録再生）だけで全機能を確かめる。
- **再現性の軸は effort**（推論の深さ）を宣言に固定して作る（現行モデルは temperature を受け付けないため）。
- **実装**：`src/harness/agent/`（役割ごとに 1 ファイル）。各ファイルの役割・組まれ方・設計は `docs/agent-code.md`。
