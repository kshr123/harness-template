...
この文書は、作業で得た気づき・使いにくかった点を、ルールへ格上げ（昇格）する前の
材料として記録する場所。同じ失敗を繰り返さないために、作業者（人・エージェント）が随時書く。
1 件＝1 項目で、冒頭に要点、続けて背景と対応。リポジトリや履歴が既に記録することは書かない。古くなったら消す。

各項目は「状態」を持つ：
- **記録のみ** … まだ昇格していない観察。
- **昇格済み（→DEC-xxxx）** … 決定を起こし、機械検査・抽象・スキル・規約のどれかへ落とした。次の見直しで消す（ただし DEC や他の learnings の**根拠として引用されている**ものは、引用先が迷子にならないよう先例として保持する＝L-004 の扱い）。
...
- **状態**：昇格済み → DEC-0002（機械検査＝一致ゲートの撤去へ落とした。次の見直しで削除してよい・method.md の先例として当面残す）。
...
- **要点**：correlations・category_target_summary・id_like・duplicate_columns を個別に読み合わせてリークを察する運用は見落としが出る。入口 1 つ（`eda.leakage_scan`＝既存部品の合成のみ・新統計なし）へ転換し、怪しい列に理由を付けて返す。止める検査にはしない（値は事実・判断は実験側）。

## L-013 案内の書き忘れ（missing link）は一方向の doclint では捕まらない
- **状態**：昇格済み → DEC-0016（案内カバレッジ検査 coverage_lint を verify に接続。DEC-0016 の根拠として引用されるため保持＝L-004 と同じく先例）。
