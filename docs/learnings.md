...
この文書は、作業で得た気づき・使いにくかった点を、ルールへ[昇格](glossary.md#昇格気づきルール)する前の
...
昇格の流れは `docs/method.md` C 節、実行手順は harvest スキル。L-001〜003・005〜006 は実装知識で昇格不要（記録のみ）。L-004 は昇格済み（→DEC-0002）で、昇格の後戻り防止（[ratchet](glossary.md#ratchet)）の先例。
