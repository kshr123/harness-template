...
- **決まりごとは手段であって目的ではない＝効かない決まりごとは撤回する**。追加の流れ（上）だけでは手段が増える一方になる。効かなくなったと分かった決まりごと（仕組み・識別子）は、追加したときと同じ手順で**撤回する**。撤回の是非は人の判断（機械にはさせない）。撤回の記録先は `docs/learnings.md`（何を・なぜ・いつ外したか）。撤回した名前が資産（src・docs・skills・templates）に残骸として残っていないかは、**撤回の作業時にその名前を横断検索して確かめる**（撤回した名前は撤回時に有限に確定する＝「確定した集合が消えたことの確認」であって「未来の不良の検出器」ではない。だから常時走らせる検査に載せず、撤回という一手の中で 1 度確かめる。手順は harvest スキル）。
