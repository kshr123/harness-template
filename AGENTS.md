...
- **業界標準のライブラリを再発明しない**。メトリクス・交差検証の分割・スケーリング等は scikit-learn、数値計算は numpy を使う。手書きは保守負債・バグの温床。差し替えたいのは「モデルの具体」と「保存形式」だけで、それは Protocol で注入する（`docs/decisions/DEC-0006`）（レビュー観点）。
