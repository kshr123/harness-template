...
[正本](docs/glossary.md#正本)（唯一の正とする置き場）。Claude Code / Codex 共通で、
Claude Code は `CLAUDE.md`（`@AGENTS.md` を取り込む）経由でこれを読む。
分からない内部用語は [docs/glossary.md](docs/glossary.md)（用語集）で引ける。

## 原則
- **完了＝検証にすべて成功したときだけ**。自己申告で完了にしない。`uv run verify` がすべて成功して初めて完了。
- **作る側と確かめる側を分ける**（[maker-checker](docs/glossary.md#maker-checker)）。実装した本人・同じ文脈のエージェントが自分で合否を判定しない。差分だけを見る独立した確認（別セッション・別エージェント、チームなら人のレビュー）を経る。重大な問題が指摘されている間は完了にしない（review スキル参照）。
- **完了↔検証の結びつけ**。`done` のタスクは、受け入れ基準を確かめるテストの場所を `verified_by` に明記する（空・指す先が無いと検査で失敗）。
- **意味のある 1 まとまり＝1 ディレクトリ**。[作業単位](docs/glossary.md#作業単位item)（エピック・タスク・実験）は `work/` の中に置き、そのまとまりのもの（説明・SPEC・コード・結果・メモ）を同じフォルダに同居させる。**親はフォルダ**（置き場所）で表す。小さい単位はフォルダでなくファイル 1 つでよい。
...
- **着手は全体→詳細**。端まで通る最小の骨組み（[歩く骨組み](docs/glossary.md#歩く骨組み)＝入力→処理→出力→検証が一巡）を先に作り、`uv run verify` を全成功に保ったまま各部を本実装で差し替える。差し替えの順番は自由（例外と理由は `docs/method.md`）。
- **進め方そのものも進化させる**。作業で得た気づきは `docs/learnings.md` に記録し、価値/一般性があると判断したら**即**（回数で待たない・DEC-0012）決定（DEC）を起こして、機械検査・抽象・スキル・規約のどれかへ[昇格](docs/glossary.md#昇格気づきルール)する（歯止めは回数でなく一般性の判断。正本と昇格の流れは `docs/method.md`）。
...
- **agent プロファイルの決定性は effort 固定＋無ネットワーク検証で作る**（agent プロファイル。`docs/decisions/DEC-0015`）。
  ...
  - verify は dummy/[cassette](docs/glossary.md#cassette)（記録再生）だけで回す（ネットワーク 0）。
  - プロバイダ既定は Anthropic。
...
- **部品は[入口](docs/glossary.md#入口)まで作って完了**。再利用する部品を作ったら、同じタスクでレジストリ登録＋docstring＋スキル/雛形からの[導線](docs/glossary.md#導線)まで更新する。エージェントが元コードを読まずに使えて初めて done（`docs/decisions/DEC-0009`。検査点：レジストリ項目の説明文必須は pytest が検査。スキルの導線はレビュー観点）。
...
- **テストに書ける数値は、テストデータの構成から導出できるものだけ**。実装の出力をそのままコピーした固定値（[金メッキ](docs/glossary.md#金メッキ)）を書かない。期待値は入力の作り方から説明できること。
