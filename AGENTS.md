...
- **業界標準のライブラリを再発明しない**。メトリクス・交差検証の分割・スケーリング等は scikit-learn、数値計算は numpy を使う。手書きは保守負債・バグの温床。差し替えたいのは「モデルの具体」（config でモデル種を選ぶ＝`MODELS` レジストリ）と「保存形式」（`ds/models.py` は丸ごと pickle・manifest の `format` 文字列で load が分岐＝可搬形式が要る時に枝を足す差し替え口）だけで、核（`ds/models.py`）は形式ライブラリを直接固定しない（`docs/decisions/DEC-0006`）（レビュー観点）。
...
- **実験スクリプトは `--test`（小さなスモーク）を必須**にし、`uv run verify` に接続する。雛形からの乖離・二重実装を機械的に防ぐ。
...
- **done の実験は結果記録（`results/` の指標・設定・データ指紋）が必須**。フォルダ単位で再現一式を同居させる（検査が空の done を失敗にする）。
